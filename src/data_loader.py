...
def compute_features(msg_i: IRCMessage, msg_j: IRCMessage, conversation: IRCConversation) -> List[float]:
    """
...
class IRCDisentanglementDataset(Dataset):
    ...
    def __init__(
        ...
        self.conversations = []
        self.conversation_map = []  # Maps pair index to (conv_idx, msg_i_idx, msg_j_idx)
        self.pairs = []  # List of (text_pair, label, features)
        
        logger.info(f"Initializing IRCDisentanglementDataset with {len(ascii_files)} files")
        logger.info(f"  max_dist={max_dist}, max_length={max_length}, is_test={is_test}")
        logger.info(f"  test_start={test_start}, test_end={test_end}")
        
        start_time = datetime.now()
        
        for idx, (ascii_path, ann_path) in enumerate(tqdm(
            ...
            logger.info(f"  File {idx+1}/{len(ascii_files)}: {conv.name} - {len(conv.messages)} messages, {len(self.pairs)} total pairs so far")
            
            # Early exit if we've reached test_end pairs (for test mode)
            if self.is_test and len(self.pairs) >= self.test_end:
                logger.info(f"  Reached test_end limit ({self.test_end} pairs), stopping early")
                break
        
        # Truncate pairs to test_end if specified (for test mode)
        if self.is_test and self.test_end < len(self.pairs):
            logger.info(f"Truncating pairs from {len(self.pairs)} to {self.test_end} (test_end limit)")
            self.pairs = self.pairs[:self.test_end]
            self.conversation_map = self.conversation_map[:self.test_end]
        
        elapsed = (datetime.now() - start_time).total_seconds()
        logger.info(f"Dataset initialization complete: {len(self.pairs)} pairs from {len(self.conversations)} conversations in {elapsed:.2f}s")
    ...
    def _create_pairs_for_conversation(self, conv: IRCConversation, conv_idx: int):
        ...
        if self.is_test:
            ...
            logger.info(f"  Creating pairs for {conv.name}: messages {start_idx} to {end_idx} (test mode)")
        else:
            process_indices = range(len(messages))
            logger.info(f"  Creating pairs for {conv.name}: all {len(messages)} messages")
        
        pairs_before = len(self.pairs)
        
        # Use tqdm for progress bar on message iteration
        for i in tqdm(process_indices, desc=f"  Pairs for {conv.name}", leave=False, disable=len(process_indices) < 100):
            msg_i = messages[i]
    ...
    def __getitem__(self, idx):
        ...
        item = {
            ...
            'labels': torch.tensor(label, dtype=torch.float32) if label != -1 else torch.tensor(0.0, dtype=torch.float32)
        }
...
def load_dataset_files(data_dir: str, split: str = 'train') -> Tuple[List[str], List[str]]:
    """
...
# Verification function to compare with original data loader
def verify_with_original():
    """Compare outputs with original read_data function"""
    print("Loading sample conversation for verification...")
    
    # Load using original pattern
    ...
    print(f"  Gold links: {len(conv.gold_links)}")
    
    # Check first few messages
    for i in range(min(5, len(conv.messages))):
        msg = conv.messages[i]
        print(f"  Message {i}: speaker={msg.speaker}, text={msg.text[:50]}...")
    
    # Check a gold link
    if conv.gold_links:
        child = list(conv.gold_links.keys())[0]
        parents = conv.gold_links[child]
        print(f"  Sample gold link: {child} -> {parents}")
        
        # Verify features
        if child < len(conv.messages) and parents[0] < len(conv.messages):
            features = compute_features(
                conv.messages[parents[0]], 
                conv.messages[child],
                conv
            )
            print(f"  Features for pair ({parents[0]}, {child}): {features}")
...
if __name__ == "__main__":
    # Quick verification
    conv = verify_with_original()
    print("\nOK Data loader verification complete")
    print("  Features: [time_diff_norm, speaker_match, pos_dist_norm, jaccard]")