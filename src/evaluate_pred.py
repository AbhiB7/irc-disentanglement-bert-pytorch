"""
IRC Conversation Disentanglement - Checkpoint Evaluation + Predicted JSON Export

Evaluates a trained checkpoint on the dev/test set, then exports predicted
links and threads to JSON files that the app/ visualizer can load.

Usage (metrics only, like evaluate.py):
    python src/evaluate_pred.py --checkpoint checkpoints/best/checkpoint_epoch_3.pt

Usage (export predicted JSONs for visualization):
    python src/evaluate_pred.py --checkpoint checkpoints/best/checkpoint_epoch_3.pt \\
        --export-json --output-dir app/predicted_data
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate IRC Disentanglement Checkpoint & Export Predicted JSONs",
    ...
    # === NEW: JSON export flags ===
    parser.add_argument(
        "--export-json",
        action="store_true",
        help="Export predicted links/threads as JSON files for visualization",
    )

    parser.add_argument(
        "--output-dir",
        type=str,
        default="app/predicted_data",
        help="Directory to write predicted JSON files to",
    )
...
# ===== NEW: JSON Export Functions =====

def build_predicted_links(predictions, dataset):
    """
    Build predicted links per conversation from model predictions.

    Returns:
        dict mapping conversation_name -> list of (child, predicted_parent)
    """
...
def build_threads_from_links(n_messages, links):
    """
    Union-Find to build thread clusters from a list of (child, parent) links.

    Args:
        n_messages: total number of messages in the conversation
        links: list of (child, parent) tuples

    Returns:
        list of dicts: [{id, messages: sorted list, size}]
    """
    uf = list(range(n_messages))
    ...
    thread_list = []
    for tid, indices in buckets.items():
        thread_list.append({
            "id": tid,
            "messages": sorted(indices),
            "size": len(indices),
        })
    thread_list.sort(key=lambda t: t["size"], reverse=True)
...
def message_to_dict(msg):
    """
    Convert an IRCMessage to a JSON-serializable dict matching
    the schema expected by app/visualize.js.

    IRCMessage fields: index, timestamp (tuple|None), speaker, text, is_system
    """
    ts = msg.timestamp
    if ts is not None:
        timestamp_str = f"{ts[0]:02d}:{ts[1]:02d}"
    else:
        timestamp_str = None
...
def export_prediction_jsons(predictions, dataset, output_dir, split):
    """
    For each conversation in the dataset, build predicted links from predictions
    + conversation_map, compute threads via union-find, and write a JSON
    file in the same format as scripts/export_chat_json.py.

    Output files: <output_dir>/predicted_<split>_<conv_name>.json

    The JSON schema matches what app/visualize.js expects:
    {name, messages: [{index, timestamp, speaker, text, is_system}],
     links: [{child, parent}], threads: [{id, messages, size}]}
    """
    # Build predicted links per conversation
    predicted_links = build_predicted_links(predictions, dataset)
    ...
    for conv in dataset.conversations:
        ...
        # Get predicted links for this conversation (or empty list if none)
        conv_links = predicted_links.get(conv_name, [])

        # Build link dicts for JSON (child, parent)
        link_dicts = [{"child": c, "parent": p} for c, p in conv_links]

        # Build predicted threads
        threads = build_threads_from_links(n_messages, conv_links)
        ...
        # Write file
        filename = f"predicted_{split}_{conv_name}.json"
        filepath = output_path / filename
        with open(filepath, "w") as f:
            json.dump(output_data, f, indent=2)

        logger.info(f"  Exported {len(messages)} msgs, {len(link_dicts)} predicted links, "
                    f"{len(threads)} threads → {filepath}")
        exported_count += 1

    logger.info(f"Exported {exported_count} conversation JSONs to {output_path}")
...
def main():
    ...
    if not args.export_json:
        # Standard mode: print metrics just like evaluate.py
        logger.info("=" * 80)
    ...
    # Clustering metrics (when not in JSON-only export mode)
    if args.metrics in ["clustering", "both"] and not args.export_json:
    ...
    # === NEW: Export predicted JSONs ===
    if args.export_json:
        logger.info("=" * 80)
        logger.info("Exporting Predicted JSONs for Visualization")
        logger.info("=" * 80)

        export_prediction_jsons(
            metrics["predictions"],
            loader.dataset,
            args.output_dir,
            args.split,
